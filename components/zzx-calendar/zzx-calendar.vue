...
<script>
	...
	export default {
		...
		watch: {
			...
			currentYear: function(newvalue) {
				// currentYear: '',
				// currentMonth: '',
				// currentDate: '',
				if (this.currentYear == newvalue) return;
				// this.$emit('swipeDate', [this.currentYear, this.currentMonth, this.currentDate].join("-"));

			},
			currentMonth: function(newvalue) {
				// if (this.currentMonth == newvalue) return;
				this.$emit('swipeDate', [this.currentYear, this.currentMonth, this.currentDate].join("-"));
			}
		},
		computed: {
